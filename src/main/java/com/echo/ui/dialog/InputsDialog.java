...
public abstract class InputsDialog extends DialogBase {
    ...
    protected void setupMainPanel(int width) {
        ...
        optionsPanel.setAlignmentX(Component.LEFT_ALIGNMENT); // Left align for consistent appearance
        optionsPanel.setBorder(new EmptyBorder(10, 10, 10, 10));

        // Add all selectors to the panel
        for (InputSelector<?> selector : selectors) {
            JPanel selectorPanel = selector.createPanel();
            selectorPanel.setAlignmentX(Component.LEFT_ALIGNMENT); // Ensure left alignment
    ...
    protected void setupBottomPanel() {
        ...
        cancelButton = new JButton(BUTTON_TEXT_CANCEL);
        cancelButton.addActionListener(e -> onCancelClicked());

        // Add continue button
        nextButton = new JButton(BUTTON_TEXT_CONTINUE);
